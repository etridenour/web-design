...
-- -- 2 Personal Website
-- SELECT * FROM project
--     JOIN project_uses_tech ON project_uses_tech.project_id = 4
--     JOIN tech ON project_uses_tech.tech_id = tech.id;

-- 3 No associated project
-- SELECT tech.name, count(tech.id) FROM project_uses_tech
...
-- 14
-- SELECT AVG(count) FROM (select tech.id, tech.name, count(tech.id) FROM tech
--     INNER JOIN project_uses_tech ON tech.id = project_uses_tech.tech_id GROUP BY tech.id) as the_stuff;
